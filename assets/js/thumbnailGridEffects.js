	...
	function init() {
		[].forEach.call( document.querySelectorAll( '.tt-grid-wrapper' ), function( el ) {
            ...
            items.forEach(function(el, i) {
                var page = ~~(i/6);
                if (allImages.length <= page)
                {
                    allImages.push([]);
                    nav.innerHTML += (page>0)?'<a></a>':'<a class="tt-current"></a>';
                }
                allImages[page].push(el.innerHTML);
                if (page > 0)
                {
            ...
            var navDots = [].slice.call( el.querySelectorAll( 'nav > a' ) );
